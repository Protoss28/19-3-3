...
username = config.username  # Имя пользователя
...
now = datetime.now(timezone.utc)  # Время в UTC для заказа
body = config.order
body['shipDate'] = now.isoformat()  # Текущее время в заказе, так же можно изменить petId и quantity
body = json.dumps(body)

res = requests.post(f'{base_url}/store/order', headers={'accept': 'application/json',
                                                        'Content-Type': 'application/json'}, data=body)

orderid_for_delete = res.json()['id']  # id заказа, что бы удалить его в следующей проверке
...
orderId = random.randint(1, 10)  # Случайный номер заказа >= 1 и <= 10
...
body['category']['name'] = 'python'  # Категория добавляемого питомца
...
body['status'] = 'test_status'  # Статус для тестирования
body = json.dumps(body)

res = requests.post(f'{base_url}/pet', headers={'accept': 'application/json',
                                                'Content-Type': 'application/json'}, data=body)

petid = res.json()['id']  # id добавленного питомца
...
body = config.new_pet
body['id'] = petid  # id питомца которого надо обновить
